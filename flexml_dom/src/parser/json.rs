// sourced from Logos example here: https://logos.maciej.codes/getting-started.html
// some minor modifications made to support flexml library
...
use crate::document::OptionValue;
...
fn parse_value(lexer: &mut Lexer<'_, Token>) -> Result<OptionValue> {
    if let Some(token) = lexer.next() {
        match token {
            Ok(Token::Bool(b)) => Ok(OptionValue::Bool(b)),
            Ok(Token::BraceOpen) => parse_object(lexer),
            Ok(Token::BracketOpen) => parse_array(lexer),
            Ok(Token::Null) => Ok(OptionValue::Null),
            Ok(Token::Number(n)) => Ok(OptionValue::Number(n)),
            Ok(Token::String(s)) => Ok(OptionValue::String(s)),
            _ => Err((
...
fn parse_array(lexer: &mut Lexer<'_, Token>) -> Result<OptionValue> {
    ...
    while let Some(token) = lexer.next() {
        match token {
            ...
            Ok(Token::BracketClose) if !awaits_value => return Ok(OptionValue::Array(array)),
            Ok(Token::Comma) if awaits_comma => awaits_value = true,
            Ok(Token::Null) if !awaits_comma => {
                array.push(OptionValue::Null);
                awaits_value = false
            }
            Ok(Token::Number(n)) if !awaits_comma => {
                array.push(OptionValue::Number(n));
                awaits_value = false;
            }
            Ok(Token::String(s)) if !awaits_comma => {
                array.push(OptionValue::String(s));
                awaits_value = false;
            }
            _ => {
                return Err((
...
///
/// > NOTE: we assume '{' was consumed.
fn parse_object(lexer: &mut Lexer<'_, Token>) -> Result<OptionValue> {
    ...
    while let Some(token) = lexer.next() {
        match token {
            Ok(Token::BraceClose) if !awaits_key => return Ok(OptionValue::Object(map)),
...
pub(crate) fn parse(src: &str) -> Result<OptionValue>{
